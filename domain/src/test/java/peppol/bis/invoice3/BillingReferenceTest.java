...
public class BillingReferenceTest {
    ...
    private BillingReference paymentTerms;

    @BeforeEach
    void setUp() {
        paymentTerms = new BillingReference(
            new InvoiceDocumentReference("inv123")
        );
    }

    @Test
    void to_xml_basic_elements() {
        final Element element = (Element) paymentTerms.node();
        assertElementNameIs(element, "BillingReference", CAC_NS);

        assertRequiredElement(element, "InvoiceDocumentReference");
    }
