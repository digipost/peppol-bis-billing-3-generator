...
public class OrderReferenceTest {
    ...
    void to_xml_basic_elements() {
        final Element element = (Element) orderReference.node();
        assertElementNameIs(element, "OrderReference", CAC_NS);

        assertRequiredElement(element, "ID", equalTo("98776"));
        assertUnsetOptionalElement(element, "SalesOrderID");
    }

    @Test
    void to_xml_optional_elements() {
        orderReference.withsalesOrderID("112233");

        final Element element = (Element) orderReference.node();

        assertRequiredElement(element, "SalesOrderID", equalTo("112233"));
    }
