...
public class PartyLegalEntityTest {
    ...
    void to_xml_required_elements() {
        final Element element = (Element) partyLegalEntity.node();
        assertElementNameIs(element, "PartyLegalEntity", CAC_NS);

        assertRequiredElement(element, "RegistrationName", equalTo("Full Formal Seller Name LTD."));

        assertUnsetOptionalElement(element, "CompanyID");
        assertUnsetOptionalElement(element, "CompanyLegalForm");
    }

    @Test
    void to_xml_optional_elements() {
        partyLegalEntity
            .withCompanyID(new CompanyID("987654321"))
            .withCompanyLegalForm("Share capital")
            ;

        final Element element = (Element) partyLegalEntity.node();

        assertRequiredElement(element, "CompanyID", equalTo("987654321"));
        assertRequiredElement(element, "CompanyLegalForm", equalTo("Share capital"));
    }
}
