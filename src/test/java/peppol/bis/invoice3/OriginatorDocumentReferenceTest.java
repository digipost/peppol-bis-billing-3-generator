...
class OriginatorDocumentReferenceTest {

    private OriginatorDocumentReference originatorDocumentReference;

    @BeforeEach
    void setUp() {
        originatorDocumentReference = new OriginatorDocumentReference(
            "PPID-123"
        );
    }
    ...
    void to_xml_basic_elements() {
        final Element element = (Element) originatorDocumentReference.node();
        assertElementNameIs(element, "OriginatorDocumentReference", CAC_NS);

        assertRequiredElement(element, "ID", equalTo("PPID-123"));
    }
}
