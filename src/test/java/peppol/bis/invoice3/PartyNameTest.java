...
public class PartyNameTest {
    ...
    @BeforeEach
    void setUp() {
        partyName = new PartyName("Deliver name");
    }

    @Test
    void to_xml_required_elements() {
        final Element element = (Element) partyName.node();
        assertElementNameIs(element, "PartyName", CAC_NS);

        assertRequiredElement(element, "Name", equalTo("Deliver name"));
    }
